...
class Hikvision_GUI(tk.Frame):
    def __init__(self, master, scroll_canvas, obj_cam
        ...
        if isinstance(folder_impil, Image.Image) == True:
            folder_impil = pil_img_resize(folder_impil, img_width = 26, img_height = 26)
        ...
        self.main_frame.grid_columnconfigure(index = 0, weight = 10)
        self.main_frame.grid_columnconfigure(index = 1, weight = 1, min = 270) #min = 312)

        self.main_frame.grid_rowconfigure(index = 0, weight = 1)

        self.left_prnt = tk.Frame(self)
        self.left_prnt['bg'] = self['bg']
        self.left_prnt.grid(column = 0, row = 0, columnspan = 1, rowspan = 1, padx = (1,1), pady = (1,5), sticky = 'nwse')

        self.right_prnt = tk.Frame(self)
        self.right_prnt['bg'] = self['bg']
        self.right_prnt.grid(column = 1, row = 0, columnspan = 1, rowspan = 1, padx = (5,1), pady = (1,5), sticky = 'nwse')

        self.left_prnt.grid_columnconfigure(index = 0, weight = 0)
        self.left_prnt.grid_columnconfigure(index = 1, weight = 0)
        self.left_prnt.grid_columnconfigure(index = 2, weight = 1)

        self.left_prnt.grid_rowconfigure(index = 0, weight = 0)
        self.left_prnt.grid_rowconfigure(index = 1, weight = 0)
        self.left_prnt.grid_rowconfigure(index = 2, weight = 1)

        self.btn_normal_cam_mode = tk.Button(self.left_prnt, relief = tk.GROOVE, text = 'Normal Camera Mode')
        self.btn_normal_cam_mode['width'] = 17
        self.btn_normal_cam_mode['command'] = self.select_GUI_1

        self.btn_SQ_cam_mode = tk.Button(self.left_prnt, relief = tk.GROOVE, text = 'SQ Camera Mode')
        ...
        self.btn_normal_cam_mode.grid(column = 0, row = 0, columnspan = 1, rowspan = 1, padx = (1,1), pady = (1,1), sticky = 'nw')
        self.btn_SQ_cam_mode.grid(column = 1, row = 0, columnspan = 1, rowspan = 1, padx = (5,1), pady = (1,1), sticky = 'nw')

        self.popout_disp_tk_status = tk.StringVar() # 'Popout Live Display: Inactive' or 'Popout Live Display: Active'
        self.popout_disp_tk_label = tk.Label(self.left_prnt, textvariable = self.popout_disp_tk_status, font = 'Helvetica 10 bold italic', fg = 'white')
        self.popout_disp_tk_label['bg'] = 'red'
        self.popout_disp_tk_label.grid(column = 0, row = 1, columnspan = 2, rowspan = 1, padx = (1,1), pady = (5,1), sticky = 'nw')
        ...
        self.right_prnt.grid_columnconfigure(index = 0, weight = 1)
        self.right_prnt.grid_rowconfigure(index = 0, weight = 1)
    ...
    def update_gui(self):
        ### function to update any existing display if required when user show this gui.
        self.obj_cam_operation.SQ_frame_display(self.cam_sq_frame_cache)
    ...
    def select_GUI_1(self):
        ...
        self.cam_disp_prnt.lift()
        self.cam_disp_sq_prnt.lower()
        self.GUI_sel_btn_state(self.btn_normal_cam_mode, self.btn_SQ_cam_mode)
    ...
    def select_GUI_2(self):
        ...
        self.cam_disp_sq_prnt.lift()
        self.cam_disp_prnt.lower()
    ...
    def record_setting_init(self):
        ...
        Validate_Float(self.rec_entry_framerate, self.rec_framerate_str, decimal_places = 2, only_positive = True, lo_limit = 1, hi_limit = 1000)
    ...
    def camera_display_GUI_1(self):
        self.cam_disp_prnt = tk.Frame(self.left_prnt)
        self.cam_disp_prnt['bg'] = 'white'
        self.cam_disp_prnt.grid(column = 0, row = 2, columnspan = 3, rowspan = 1, padx = (1,1), pady = (1,1), sticky = 'nwse')
    ...
    def custom_mouse_enter_SQ_disp(self, event, disp_scroll_class, scroll_hbar = True):
        if scroll_hbar == True:
            disp_scroll_class.show(scroll_x = False, scroll_y = True)
    ...
    def custom_mouse_leave_SQ_disp(self, event, disp_scroll_class, scroll_hbar = False):
        if scroll_hbar == False:
            disp_scroll_class.show(scroll_x = False, scroll_y = False)

        self.scroll_canvas._bound_to_mousewheel(event)

    def camera_display_GUI_2(self):
        self.cam_disp_sq_prnt = tk.Frame(self.left_prnt)
        self.cam_disp_sq_prnt['bg'] = 'white'
        self.cam_disp_sq_prnt.grid(column = 0, row = 2, columnspan = 3, rowspan = 1, padx = (1,1), pady = (1,1), sticky = 'nwse')
        ...
        self.SQ_frame_scroll_display.show(scroll_x = False, scroll_y = False) #To Place the Scroll Canvas
        ...
        for i in range(0, 16):
            ...
            tk_disp.bind("<Configure>", partial(self.sq_img_resize, tk_disp = tk_disp, disp_id = int(i)))
            tk_disp.update_idletasks()

            self.tk_sq_disp_list.append(tk_disp)
            self.tk_cam_sq_widget_list.append((tk_lb, tk_disp))

    def sq_img_resize(self, event, tk_disp, disp_id):
        disp_id = int(disp_id)
        if len(self.cam_sq_frame_cache) > 0:
            try:
                display_func(tk_disp, self.cam_sq_frame_cache[disp_id], 'img', force_update = True)
            except IndexError:
                pass
    ...
    def camera_control_GUI(self):
        ...
        self.cam_ctrl_frame = tk.Frame(self.right_prnt, bg = bg_color, highlightbackground = 'navy', highlightthickness=1)
        self.cam_ctrl_frame.grid(column = 0, row = 0, columnspan = 1, rowspan = 1, padx = (1,1), pady = (1,1), sticky = 'nwse')

        self.cam_ctrl_frame.grid_columnconfigure(index = 0, weight = 1)
        self.cam_ctrl_frame.grid_rowconfigure(index = 0, weight = 1, min = 150)
        self.cam_ctrl_frame.grid_rowconfigure(index = 1, weight = 1, min = 120)
        self.cam_ctrl_frame.grid_rowconfigure(index = 2, weight = 1, min = 348 + 100 + 70)
        ...
        self.cam_frame_1.grid(column = 0, row = 0, columnspan = 1, rowspan = 1, padx = (5,5), pady = (5,1), sticky = 'nwse')
        self.cam_frame_1.update_idletasks()
        cam_ctrl_fr_H = cam_ctrl_fr_H + 5 + self.cam_frame_1.winfo_height()

        self.cam_frame_2.grid(column = 0, row = 1, columnspan = 1, rowspan = 1, padx = (5,5), pady = (5,1), sticky = 'nwse')
        self.cam_frame_2.update_idletasks()
        cam_ctrl_fr_H = cam_ctrl_fr_H + 5 + self.cam_frame_2.winfo_height()

        self.cam_frame_3.grid(column = 0, row = 2, columnspan = 1, rowspan = 1, padx = (5,5), pady = (5,5), sticky = 'nwse')
    ...
    def open_device(self, hikvision_devList, nSelCamIndex = 0):
        ...
        if len(hikvision_devList) > 0:
            ...
            if  0 != ret:
                ...
                self.cam_connect_class.conn_popout_close()
