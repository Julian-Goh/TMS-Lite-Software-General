...
class LC18_16CH_GUI(tk.Frame):
    def __init__(self, master, dll_LC18, light_conn_status, firmware_model_sel, firmware_version_str, infinity_icon
        ...
        self.channel_5_save = np.zeros ((arr_size),dtype=np.uint16)
        self.channel_6_save = np.zeros ((arr_size),dtype=np.uint16)
        self.channel_7_save = np.zeros ((arr_size),dtype=np.uint16)
        self.channel_8_save = np.zeros ((arr_size),dtype=np.uint16)
        self.channel_9_save = np.zeros ((arr_size),dtype=np.uint16)
        self.channel_10_save = np.zeros ((arr_size),dtype=np.uint16)
        self.channel_11_save = np.zeros ((arr_size),dtype=np.uint16)
        self.channel_12_save = np.zeros ((arr_size),dtype=np.uint16)
        self.channel_13_save = np.zeros ((arr_size),dtype=np.uint16)
        self.channel_14_save = np.zeros ((arr_size),dtype=np.uint16)
        self.channel_15_save = np.zeros ((arr_size),dtype=np.uint16)
        self.channel_16_save = np.zeros ((arr_size),dtype=np.uint16)
        ...
        self.channel_5_save[5] = self.channel_6_save[5] = self.channel_7_save[5] = self.channel_8_save[5] = 100 
        self.channel_5_save[7] = self.channel_6_save[7] = self.channel_7_save[7] = self.channel_8_save[7] = 0

        self.channel_9_save[3] = self.channel_10_save[3] = self.channel_11_save[3] = self.channel_12_save[3] = 100 
        self.channel_9_save[5] = self.channel_10_save[5] = self.channel_11_save[5] = self.channel_12_save[5] = 100 
        self.channel_9_save[7] = self.channel_10_save[7] = self.channel_11_save[7] = self.channel_12_save[7] = 0

        self.channel_13_save[3] = self.channel_14_save[3] = self.channel_15_save[3] = self.channel_16_save[3] = 100 
        self.channel_13_save[5] = self.channel_14_save[5] = self.channel_15_save[5] = self.channel_16_save[5] = 100 
        self.channel_13_save[7] = self.channel_14_save[7] = self.channel_15_save[7] = self.channel_16_save[7] = 0
    ...
    def channel_on_select(self):
        ...
        self.strobe_ch_sel()
        widget_enable(self.sel_1_checkbox, self.sel_2_checkbox, self.sel_3_checkbox, self.sel_4_checkbox, self.sel_5_checkbox, self.sel_6_checkbox, self.sel_7_checkbox, self.sel_8_checkbox
        ...
        if self.ch_sel_str == '1 - 4':
            ...
            (self.ch_5_entry_c_label, self.ch_5_entry_d_label, self.ch_5_entry_e_label, self.ch_5_entry_f_label, self.ch_5_mode, 
                self.ch_5_scalevar_a, self.ch_5_scalevar_b, self.ch_5_scalevar_c, self.ch_5_scalevar_d, self.ch_5_scalevar_e, self.ch_5_scalevar_f) = self.generate_panel(self.channel_frame_5,'Channel 5', 5, self.channel_5_save)

            self.channel_frame_6 = self.generate_ch_frame(self.frame_panel_5, 6)
            (self.ch_6_entry_c_label, self.ch_6_entry_d_label, self.ch_6_entry_e_label, self.ch_6_entry_f_label, self.ch_6_mode, 
                self.ch_6_scalevar_a, self.ch_6_scalevar_b, self.ch_6_scalevar_c, self.ch_6_scalevar_d, self.ch_6_scalevar_e, self.ch_6_scalevar_f) = self.generate_panel(self.channel_frame_6,'Channel 6', 6, self.channel_6_save)

            self.channel_frame_7 = self.generate_ch_frame(self.frame_panel_5, 7)
            (self.ch_7_entry_c_label, self.ch_7_entry_d_label, self.ch_7_entry_e_label, self.ch_7_entry_f_label, self.ch_7_mode, 
                self.ch_7_scalevar_a, self.ch_7_scalevar_b, self.ch_7_scalevar_c, self.ch_7_scalevar_d, self.ch_7_scalevar_e, self.ch_7_scalevar_f) = self.generate_panel(self.channel_frame_7,'Channel 7', 7, self.channel_7_save)

            self.channel_frame_8 = self.generate_ch_frame(self.frame_panel_5, 8)
            (self.ch_8_entry_c_label, self.ch_8_entry_d_label, self.ch_8_entry_e_label, self.ch_8_entry_f_label, self.ch_8_mode, 
                self.ch_8_scalevar_a, self.ch_8_scalevar_b, self.ch_8_scalevar_c, self.ch_8_scalevar_d, self.ch_8_scalevar_e, self.ch_8_scalevar_f) = self.generate_panel(self.channel_frame_8,'Channel 8', 8, self.channel_8_save)
            ...
            (self.ch_9_entry_c_label, self.ch_9_entry_d_label, self.ch_9_entry_e_label, self.ch_9_entry_f_label, self.ch_9_mode, 
                self.ch_9_scalevar_a, self.ch_9_scalevar_b, self.ch_9_scalevar_c, self.ch_9_scalevar_d, self.ch_9_scalevar_e, self.ch_9_scalevar_f) = self.generate_panel(self.channel_frame_9,'Channel 9', 9, self.channel_9_save)

            self.channel_frame_10 = self.generate_ch_frame(self.frame_panel_5, 10)
            (self.ch_10_entry_c_label, self.ch_10_entry_d_label, self.ch_10_entry_e_label, self.ch_10_entry_f_label, self.ch_10_mode, 
                self.ch_10_scalevar_a, self.ch_10_scalevar_b, self.ch_10_scalevar_c, self.ch_10_scalevar_d, self.ch_10_scalevar_e, self.ch_10_scalevar_f) = self.generate_panel(self.channel_frame_10,'Channel 10', 10, self.channel_10_save)

            self.channel_frame_11 = self.generate_ch_frame(self.frame_panel_5, 11)
            (self.ch_11_entry_c_label, self.ch_11_entry_d_label, self.ch_11_entry_e_label, self.ch_11_entry_f_label, self.ch_11_mode, 
                self.ch_11_scalevar_a, self.ch_11_scalevar_b, self.ch_11_scalevar_c, self.ch_11_scalevar_d, self.ch_11_scalevar_e, self.ch_11_scalevar_f) = self.generate_panel(self.channel_frame_11,'Channel 11', 11, self.channel_11_save)

            self.channel_frame_12 = self.generate_ch_frame(self.frame_panel_5, 12)
            (self.ch_12_entry_c_label, self.ch_12_entry_d_label, self.ch_12_entry_e_label, self.ch_12_entry_f_label, self.ch_12_mode, 
                self.ch_12_scalevar_a, self.ch_12_scalevar_b, self.ch_12_scalevar_c, self.ch_12_scalevar_d, self.ch_12_scalevar_e, self.ch_12_scalevar_f) = self.generate_panel(self.channel_frame_12,'Channel 12', 12, self.channel_12_save)
            ...
            (self.ch_13_entry_c_label, self.ch_13_entry_d_label, self.ch_13_entry_e_label, self.ch_13_entry_f_label, self.ch_13_mode, 
                self.ch_13_scalevar_a, self.ch_13_scalevar_b, self.ch_13_scalevar_c, self.ch_13_scalevar_d, self.ch_13_scalevar_e, self.ch_13_scalevar_f) = self.generate_panel(self.channel_frame_13,'Channel 13', 13, self.channel_13_save)

            self.channel_frame_14 = self.generate_ch_frame(self.frame_panel_5, 14)
            (self.ch_14_entry_c_label, self.ch_14_entry_d_label, self.ch_14_entry_e_label, self.ch_14_entry_f_label, self.ch_14_mode, 
                self.ch_14_scalevar_a, self.ch_14_scalevar_b, self.ch_14_scalevar_c, self.ch_14_scalevar_d, self.ch_14_scalevar_e, self.ch_14_scalevar_f) = self.generate_panel(self.channel_frame_14,'Channel 14', 14, self.channel_14_save)

            self.channel_frame_15 = self.generate_ch_frame(self.frame_panel_5, 15)
            (self.ch_15_entry_c_label, self.ch_15_entry_d_label, self.ch_15_entry_e_label, self.ch_15_entry_f_label, self.ch_15_mode, 
                self.ch_15_scalevar_a, self.ch_15_scalevar_b, self.ch_15_scalevar_c, self.ch_15_scalevar_d, self.ch_15_scalevar_e, self.ch_15_scalevar_f) = self.generate_panel(self.channel_frame_15,'Channel 15', 15, self.channel_15_save)

            self.channel_frame_16 = self.generate_ch_frame(self.frame_panel_5, 16)
            (self.ch_16_entry_c_label, self.ch_16_entry_d_label, self.ch_16_entry_e_label, self.ch_16_entry_f_label, self.ch_16_mode, 
                self.ch_16_scalevar_a, self.ch_16_scalevar_b, self.ch_16_scalevar_c, self.ch_16_scalevar_d, self.ch_16_scalevar_e, self.ch_16_scalevar_f) = self.generate_panel(self.channel_frame_16,'Channel 16', 16, self.channel_16_save)
            pass
    ...
    def board_address(self):
        ...
        self.addr_index_b = binary_to_dec_v2(self.addr_b[0], self.addr_b[1], self.addr_b[2], self.addr_b[3], reverse_str = True)
        self.addr_index_c = binary_to_dec_v2(self.addr_c[0], self.addr_c[1], self.addr_c[2], self.addr_c[3], reverse_str = True)
        self.addr_index_d = binary_to_dec_v2(self.addr_d[0], self.addr_d[1], self.addr_d[2], self.addr_d[3], reverse_str = True)
        ...
        elif self.ch_sel_str == '5 - 8': #0001
            (self.addr_0_checkbox_b, self.addr_1_checkbox_b, 
                self.addr_2_checkbox_b, self.addr_3_checkbox_b) = self.board_addr_checkbox(self.addr_b[0], self.addr_b[1], self.addr_b[2], self.addr_b[3])
            self.addr_0_checkbox_b.place(x= 10, y = y_addr_checkbox)
            self.addr_1_checkbox_b.place(x= 35, y = y_addr_checkbox)
            self.addr_2_checkbox_b.place(x= 60, y = y_addr_checkbox)
            self.addr_3_checkbox_b.place(x= 85, y = y_addr_checkbox)


        elif self.ch_sel_str == '9 - 12': #0010
            (self.addr_0_checkbox_c, self.addr_1_checkbox_c, 
                self.addr_2_checkbox_c, self.addr_3_checkbox_c) = self.board_addr_checkbox(self.addr_c[0], self.addr_c[1], self.addr_c[2], self.addr_c[3])
            self.addr_0_checkbox_c.place(x= 10, y = y_addr_checkbox)
            self.addr_1_checkbox_c.place(x= 35, y = y_addr_checkbox)
            self.addr_2_checkbox_c.place(x= 60, y = y_addr_checkbox)
            self.addr_3_checkbox_c.place(x= 85, y = y_addr_checkbox)


        elif self.ch_sel_str == '13 - 16': #0011
            (self.addr_0_checkbox_d, self.addr_1_checkbox_d, 
                self.addr_2_checkbox_d, self.addr_3_checkbox_d) = self.board_addr_checkbox(self.addr_d[0], self.addr_d[1], self.addr_d[2], self.addr_d[3])
            self.addr_0_checkbox_d.place(x= 10, y = y_addr_checkbox)
            self.addr_1_checkbox_d.place(x= 35, y = y_addr_checkbox)
            self.addr_2_checkbox_d.place(x= 60, y = y_addr_checkbox)
            self.addr_3_checkbox_d.place(x= 85, y = y_addr_checkbox)
    ...
    def board_addr_click(self,event = None):
        if self.ch_sel_str == '1 - 4':
            ...
            self.addr_b[0] = self.addr_0_status.get()
            self.addr_b[1] = self.addr_1_status.get()
            self.addr_b[2] = self.addr_2_status.get()
            self.addr_b[3] = self.addr_3_status.get()
            self.addr_index_b = binary_to_dec_v2(self.addr_b[0], self.addr_b[1], self.addr_b[2], self.addr_b[3], reverse_str = True)
            self.channel_5_save[7] = self.channel_6_save[7] = self.channel_7_save[7] = self.channel_8_save[7] = self.addr_index_b
            
        elif self.ch_sel_str == '9 - 12':
            self.addr_c[0] = self.addr_0_status.get()
            self.addr_c[1] = self.addr_1_status.get()
            self.addr_c[2] = self.addr_2_status.get()
            self.addr_c[3] = self.addr_3_status.get()
            self.addr_index_c = binary_to_dec_v2(self.addr_c[0], self.addr_c[1], self.addr_c[2], self.addr_c[3], reverse_str = True)
            self.channel_9_save[7] = self.channel_10_save[7] = self.channel_11_save[7] = self.channel_12_save[7] = self.addr_index_c

        elif self.ch_sel_str == '13 - 16':
            self.addr_d[0] = self.addr_0_status.get()
            self.addr_d[1] = self.addr_1_status.get()
            self.addr_d[2] = self.addr_2_status.get()
            self.addr_d[3] = self.addr_3_status.get()
            self.addr_index_d = binary_to_dec_v2(self.addr_d[0], self.addr_d[1], self.addr_d[2], self.addr_d[3], reverse_str = True)
            self.channel_13_save[7] = self.channel_14_save[7] = self.channel_15_save[7] = self.channel_16_save[7] = self.addr_index_d

        # print('addr_a: ', self.addr_a)
        # print('addr_b: ', self.addr_b)
        # print('addr_c: ', self.addr_c)
        # print('addr_d: ', self.addr_d)
    ...
    def strobe_button_command(self, ch_tag_num, widget):
        ...
        elif ch_tag_num == 5 or ch_tag_num == 6 or ch_tag_num == 7 or ch_tag_num == 8:
            widget['command'] = lambda: self.strobe_button_ch5_8(ch_tag_num)

        elif ch_tag_num == 9 or ch_tag_num == 10 or ch_tag_num == 11 or ch_tag_num == 12:
            widget['command'] = lambda: self.strobe_button_ch9_12(ch_tag_num)

        elif ch_tag_num == 13 or ch_tag_num == 14 or ch_tag_num == 15 or ch_tag_num == 16:
            widget['command'] = lambda: self.strobe_button_ch13_16(ch_tag_num)
    ...
    def current_multi_group_A(self, event=None):
        ...
        if self.ch_sel_str == '1 - 4':
            ...
            self.param_to_machine(1, self.channel_5_save)

        elif self.ch_sel_str== '9 - 12':
            self.current_multi_func(self.channel_9_save, self.ch_9_scalevar_a)
            self.param_to_machine(1, self.channel_9_save)

        elif self.ch_sel_str== '13 - 16':
            self.current_multi_func(self.channel_13_save, self.ch_13_scalevar_a)
            self.param_to_machine(1, self.channel_13_save)
    ...
    def strobe_param_group_A(self, event=None):
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.strobe_param_func(self.channel_5_save, self.ch_5_entry_c_label, self.ch_5_entry_d_label, 
                self.ch_5_scalevar_c, self.ch_5_scalevar_d)
            self.param_to_machine(1, self.channel_5_save)

        elif self.ch_sel_str== '9 - 12':
            self.strobe_param_func(self.channel_9_save, self.ch_9_entry_c_label, self.ch_9_entry_d_label, 
                self.ch_9_scalevar_c, self.ch_9_scalevar_d)
            self.param_to_machine(1, self.channel_9_save)

        elif self.ch_sel_str== '13 - 16':
            self.strobe_param_func(self.channel_13_save, self.ch_13_entry_c_label, self.ch_13_entry_d_label, 
    ...
    def strobe_param_group_B(self, event=None):
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.strobe_param_func(self.channel_6_save, self.ch_6_entry_c_label, self.ch_6_entry_d_label, 
                self.ch_6_scalevar_c, self.ch_6_scalevar_d)
            self.param_to_machine(2, self.channel_6_save)

        elif self.ch_sel_str== '9 - 12':
            self.strobe_param_func(self.channel_10_save, self.ch_10_entry_c_label, self.ch_10_entry_d_label, 
                self.ch_10_scalevar_c, self.ch_10_scalevar_d)
            self.param_to_machine(2, self.channel_10_save)

        elif self.ch_sel_str== '13 - 16':
            self.strobe_param_func(self.channel_14_save, self.ch_14_entry_c_label, self.ch_14_entry_d_label, 
    ...
    def strobe_param_group_C(self, event=None):
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.strobe_param_func(self.channel_7_save, self.ch_7_entry_c_label, self.ch_7_entry_d_label,
                self.ch_7_scalevar_c, self.ch_7_scalevar_d)
            self.param_to_machine(3, self.channel_7_save)

        elif self.ch_sel_str== '9 - 12':
            self.strobe_param_func(self.channel_11_save, self.ch_11_entry_c_label, self.ch_11_entry_d_label,
                self.ch_11_scalevar_c, self.ch_11_scalevar_d)
            self.param_to_machine(3, self.channel_11_save)

        elif self.ch_sel_str== '13 - 16':
            self.strobe_param_func(self.channel_15_save, self.ch_15_entry_c_label, self.ch_15_entry_d_label,
    ...
    def strobe_param_group_D(self, event=None):
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.strobe_param_func(self.channel_8_save, self.ch_8_entry_c_label, self.ch_8_entry_d_label,
                self.ch_8_scalevar_c, self.ch_8_scalevar_d)
            self.param_to_machine(4, self.channel_8_save)

        elif self.ch_sel_str== '9 - 12':
            self.strobe_param_func(self.channel_12_save, self.ch_12_entry_c_label, self.ch_12_entry_d_label,
                self.ch_12_scalevar_c, self.ch_12_scalevar_d)
            self.param_to_machine(4, self.channel_12_save)

        elif self.ch_sel_str== '13 - 16':
            self.strobe_param_func(self.channel_16_save, self.ch_16_entry_c_label, self.ch_16_entry_d_label,
    ...
    def output_param_group_A(self, event=None):
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.output_param_func(self.channel_5_save, self.ch_5_entry_e_label, self.ch_5_entry_f_label,
                self.ch_5_scalevar_e, self.ch_5_scalevar_f)

            self.param_to_machine(1, self.channel_5_save)

        elif self.ch_sel_str== '9 - 12':
            self.output_param_func(self.channel_9_save, self.ch_9_entry_e_label, self.ch_9_entry_f_label,
                self.ch_9_scalevar_e, self.ch_9_scalevar_f)

            self.param_to_machine(1, self.channel_9_save)

        elif self.ch_sel_str== '13 - 16':
            self.output_param_func(self.channel_13_save, self.ch_13_entry_e_label, self.ch_13_entry_f_label,
                self.ch_13_scalevar_e, self.ch_13_scalevar_f)
    ...
    def output_param_group_B(self, event=None):
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.output_param_func(self.channel_6_save, self.ch_6_entry_e_label, self.ch_6_entry_f_label,
                self.ch_6_scalevar_e, self.ch_6_scalevar_f)

            self.param_to_machine(2, self.channel_6_save)

        elif self.ch_sel_str== '9 - 12':
            self.output_param_func(self.channel_10_save, self.ch_10_entry_e_label, self.ch_10_entry_f_label,
                self.ch_10_scalevar_e, self.ch_10_scalevar_f)

            self.param_to_machine(2, self.channel_10_save)

        elif self.ch_sel_str== '13 - 16':
            self.output_param_func(self.channel_14_save, self.ch_14_entry_e_label, self.ch_14_entry_f_label,
                self.ch_14_scalevar_e, self.ch_14_scalevar_f)
    ...
    def output_param_group_C(self, event=None):
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.output_param_func(self.channel_7_save, self.ch_7_entry_e_label, self.ch_7_entry_f_label,
                self.ch_7_scalevar_e, self.ch_7_scalevar_f)

            self.param_to_machine(3, self.channel_7_save)

        elif self.ch_sel_str== '9 - 12':
            self.output_param_func(self.channel_11_save, self.ch_11_entry_e_label, self.ch_11_entry_f_label,
                self.ch_11_scalevar_e, self.ch_11_scalevar_f)

            self.param_to_machine(3, self.channel_11_save)

        elif self.ch_sel_str== '13 - 16':
            self.output_param_func(self.channel_15_save, self.ch_15_entry_e_label, self.ch_15_entry_f_label,
                self.ch_15_scalevar_e, self.ch_15_scalevar_f)
    ...
    def output_param_group_D(self, event=None):
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.output_param_func(self.channel_8_save, self.ch_8_entry_e_label, self.ch_8_entry_f_label,
                self.ch_8_scalevar_e, self.ch_8_scalevar_f)

            self.param_to_machine(4, self.channel_8_save)

        elif self.ch_sel_str== '9 - 12':
            self.output_param_func(self.channel_12_save, self.ch_12_entry_e_label, self.ch_12_entry_f_label,
                self.ch_12_scalevar_e, self.ch_12_scalevar_f)

            self.param_to_machine(4, self.channel_12_save)

        elif self.ch_sel_str== '13 - 16':
            self.output_param_func(self.channel_16_save, self.ch_16_entry_e_label, self.ch_16_entry_f_label,
                self.ch_16_scalevar_e, self.ch_16_scalevar_f)
    ...
    def mode_select_group_A(self,event=None): #group A (channel 1, 5, 9, 13)
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.mode_function(self.ch_5_mode, self.channel_5_save)
            self.param_to_machine(1, self.channel_5_save)

        elif self.ch_sel_str== '9 - 12':
            self.mode_function(self.ch_9_mode, self.channel_9_save)
            self.param_to_machine(1, self.channel_9_save)

        elif self.ch_sel_str== '13 - 16':
            self.mode_function(self.ch_13_mode, self.channel_13_save)
            self.param_to_machine(1, self.channel_13_save)
    ...
    def mode_select_group_B(self,event=None): #group B (channel 2, 6, 10, 14)
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.mode_function(self.ch_6_mode, self.channel_6_save)
            self.param_to_machine(2, self.channel_6_save)

        elif self.ch_sel_str== '9 - 12':
            self.mode_function(self.ch_10_mode, self.channel_10_save)
            self.param_to_machine(2, self.channel_10_save)

        elif self.ch_sel_str== '13 - 16':
            self.mode_function(self.ch_14_mode, self.channel_14_save)
            self.param_to_machine(2, self.channel_14_save)
    ...
    def mode_select_group_C(self,event=None): #group C (channel 3, 7, 11, 15)
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.mode_function(self.ch_7_mode, self.channel_7_save)
            self.param_to_machine(3, self.channel_7_save)

        elif self.ch_sel_str== '9 - 12':
            self.mode_function(self.ch_11_mode, self.channel_11_save)
            self.param_to_machine(3, self.channel_11_save)

        elif self.ch_sel_str== '13 - 16':
            self.mode_function(self.ch_15_mode, self.channel_15_save)
            self.param_to_machine(3, self.channel_15_save)
    ...
    def mode_select_group_D(self,event=None): #group D (channel 4, 8, 12, 16)
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.mode_function(self.ch_8_mode, self.channel_8_save)
            self.param_to_machine(4, self.channel_8_save)

        elif self.ch_sel_str== '9 - 12':
            self.mode_function(self.ch_12_mode, self.channel_12_save)
            self.param_to_machine(4, self.channel_12_save)

        elif self.ch_sel_str== '13 - 16':
            self.mode_function(self.ch_16_mode, self.channel_16_save)
            self.param_to_machine(4, self.channel_16_save)
    ...
    def strobe_button_ch5_8(self, ch_tag_num):
        self.ctrl.select_address(self.addr_index_b)
        ch_set = ch_tag_num - 4
        self.ctrl.strobe(ch_set)

    def strobe_button_ch9_12(self, ch_tag_num):
        self.ctrl.select_address(self.addr_index_c)
        ch_set = ch_tag_num - 8
        self.ctrl.strobe(ch_set)

    def strobe_button_ch13_16(self, ch_tag_num):
        self.ctrl.select_address(self.addr_index_d)
        ch_set = ch_tag_num - 12
        self.ctrl.strobe(ch_set)
    ...
    def strobe_channel_repeat(self):
        ...
        self.ctrl.select_address(self.addr_index_b)
        if self.channel_5_save[6] == 1 and self.strobe_ch_arr[4] == 1:
            self.ctrl.strobe(1)

        if self.channel_6_save[6] == 1 and self.strobe_ch_arr[5] == 1:
            self.ctrl.strobe(2)

        if self.channel_7_save[6] == 1 and self.strobe_ch_arr[6] == 1:
            self.ctrl.strobe(3)

        if self.channel_8_save[6] == 1 and self.strobe_ch_arr[7] == 1:
            self.ctrl.strobe(4)

        self.ctrl.select_address(self.addr_index_c)
        if self.channel_9_save[6] == 1 and self.strobe_ch_arr[8] == 1:
            self.ctrl.strobe(1)

        if self.channel_10_save[6] == 1 and self.strobe_ch_arr[9] == 1:
            self.ctrl.strobe(2)

        if self.channel_11_save[6] == 1 and self.strobe_ch_arr[10] == 1:
            self.ctrl.strobe(3)

        if self.channel_12_save[6] == 1 and self.strobe_ch_arr[11] == 1:
            self.ctrl.strobe(4)

        self.ctrl.select_address(self.addr_index_d)
        if self.channel_13_save[6] == 1 and self.strobe_ch_arr[12] == 1:
            self.ctrl.strobe(1)

        if self.channel_14_save[6] == 1 and self.strobe_ch_arr[13] == 1:
            self.ctrl.strobe(2)

        if self.channel_15_save[6] == 1 and self.strobe_ch_arr[14] == 1:
            self.ctrl.strobe(3)

        if self.channel_16_save[6] == 1 and self.strobe_ch_arr[15] == 1:
            self.ctrl.strobe(4)
    ...
    def strobe_channel_repeat_ALL(self):
        ...
        self.ctrl.select_address(self.addr_index_b)
        if self.channel_5_save[6] == 1:
            self.ctrl.strobe(1)

        if self.channel_6_save[6] == 1:
            self.ctrl.strobe(2)

        if self.channel_7_save[6] == 1:
            self.ctrl.strobe(3)

        if self.channel_8_save[6] == 1:
            self.ctrl.strobe(4)

        self.ctrl.select_address(self.addr_index_c)
        if self.channel_9_save[6] == 1:
            self.ctrl.strobe(1)

        if self.channel_10_save[6] == 1:
            self.ctrl.strobe(2)

        if self.channel_11_save[6] == 1:
            self.ctrl.strobe(3)

        if self.channel_12_save[6] == 1:
            self.ctrl.strobe(4)

        self.ctrl.select_address(self.addr_index_d)
        if self.channel_13_save[6] == 1:
            self.ctrl.strobe(1)

        if self.channel_14_save[6] == 1:
            self.ctrl.strobe(2)

        if self.channel_15_save[6] == 1:
            self.ctrl.strobe(3)

        if self.channel_16_save[6] == 1:
            self.ctrl.strobe(4)
    ...
    def reset_all(self): #reset everything
        ...
        self.reset_save_arr(self.channel_5_save)
        self.reset_save_arr(self.channel_6_save)
        self.reset_save_arr(self.channel_7_save)
        self.reset_save_arr(self.channel_8_save)
        self.reset_save_arr(self.channel_9_save)
        self.reset_save_arr(self.channel_10_save)
        self.reset_save_arr(self.channel_11_save)
        self.reset_save_arr(self.channel_12_save)
        self.reset_save_arr(self.channel_13_save)
        self.reset_save_arr(self.channel_14_save)
        self.reset_save_arr(self.channel_15_save)
        self.reset_save_arr(self.channel_16_save)
        ...
        try:
            if self.light_conn_status == True:
                ...
                self.reset_function(self.channel_5_save, 1)
                self.reset_function(self.channel_6_save, 2)
                self.reset_function(self.channel_7_save, 3)
                self.reset_function(self.channel_8_save, 4)
                self.reset_function(self.channel_9_save, 1)
                self.reset_function(self.channel_10_save, 2)
                self.reset_function(self.channel_11_save, 3)
                self.reset_function(self.channel_12_save, 4)
                self.reset_function(self.channel_13_save, 1)
                self.reset_function(self.channel_14_save, 2)
                self.reset_function(self.channel_15_save, 3)
                self.reset_function(self.channel_16_save, 4)
    ...
    def load_parameter(self, event=None):
        ...
        try:
            ...
            self.ctrl.select_address(self.addr_index_b)
            self.ctrl.read_function(self.channel_5_save, 1)
            self.ctrl.read_function(self.channel_6_save, 2)
            self.ctrl.read_function(self.channel_7_save, 3)
            self.ctrl.read_function(self.channel_8_save, 4)

            self.ctrl.select_address(self.addr_index_c)
            self.ctrl.read_function(self.channel_9_save, 1)
            self.ctrl.read_function(self.channel_10_save, 2)
            self.ctrl.read_function(self.channel_11_save, 3)
            self.ctrl.read_function(self.channel_12_save, 4)

            self.ctrl.select_address(self.addr_index_d)
            self.ctrl.read_function(self.channel_13_save, 1)
            self.ctrl.read_function(self.channel_14_save, 2)
            self.ctrl.read_function(self.channel_15_save, 3)
            self.ctrl.read_function(self.channel_16_save, 4)
