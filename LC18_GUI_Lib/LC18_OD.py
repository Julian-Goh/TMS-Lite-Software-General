...
class LC18_OD_GUI(tk.Frame):
    ...
    def generate_panel(self, channel_frame, channel_name, ch_tag_num, channel_save):
        ...
        #####################################################################################################################
        ch_label_d = tk.Label(channel_frame, text = 'Strobe Width\n(0-1000)', font = 'Helvetica 11', width = 12)
        scalevar_d = tk.IntVar(value = channel_save[3])
        ch_entry_d = tk.Spinbox(master = channel_frame, width = 4, textvariable = scalevar_d, from_=0, to= 1000, increment = 1
                             , highlightbackground="black", highlightthickness=1, font = 'Helvetica 11')
        ch_entry_d['validate']='key'
        ch_entry_d['vcmd']=(ch_entry_c.register(validate_int_entry),'%d','%P','%S', True)

        ch_scalebar_d = tk.Scale(channel_frame, from_=0, to=1000, variable=scalevar_d, orient='horizontal', showvalue=0)
    ...
    def Stop_Threads(self):
        ...
        try:
            ...
            pass
    ###############################################################################################
    #9. SPECIAL FUNCTIONS (LC18 OD)
    def group_strobe_offset(self):
        self.channel_1_save = self.strobe_offset(self.channel_1_save, 1)
        self.channel_2_save = self.strobe_offset(self.channel_2_save, 2)
        self.channel_3_save = self.strobe_offset(self.channel_3_save, 3)
        self.channel_4_save = self.strobe_offset(self.channel_4_save, 4)

    def strobe_offset(self,ch_save, ch_index):
        if ch_save[3] > 1000:
            ch_save[3] = 1000
            self.ctrl.select_address(ch_save[7])
            self.ctrl.set_strobe_delay(ch_index, ch_save[2])
            self.ctrl.set_strobe_width(ch_index, ch_save[3])

        if ch_save[6] != 1:
            ch_save[6] = 1
            self.ctrl.select_address(ch_save[7])
            self.ctrl.set_mode(ch_index, ch_save[6])
        else:
            pass
        return ch_save

    def group_strobe_mode(self):
        self.channel_1_save[6] = self.channel_2_save[6] = self.channel_3_save[6] = self.channel_4_save[6] = 1