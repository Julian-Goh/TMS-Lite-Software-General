...
class LC18_RGBW_GUI(tk.Frame):
    ...
    def repeat_ALL_control_gen(self):
        ...
        self.repeat_ALL_frame.place(x = 5, y = 385)
    ...
    def channel_on_select(self):
        ...
        if self.ch_sel_str == '1 - 4':
            ...
            self.total_intensity_frame = tk.Frame(self.frame_panel_1, highlightcolor = 'white', bg = 'DarkSlateGray2', highlightbackground = 'white',  highlightthickness = 1)
            self.total_intensity_frame['width'] = 120 + 5 #168

            self.total_intensity_frame['height'] = 150 #150
            self.total_intensity_frame.place(x=5, y=220)

            tk.Label(self.total_intensity_frame, text = 'Total Intensity', font = 'Helvetica 12 bold', bg = 'DarkSlateGray2').place(x=0, y=0)
            self.percentage_intensity_function(self.channel_1_save, self.channel_2_save, self.channel_3_save, self.channel_4_save)
    ...
    def current_multi_group_A(self, event=None):
        ...
        if self.ch_sel_str == '1 - 4':
            ...
            self.channel_2_save[0] = self.channel_1_save[0]
            self.channel_3_save[0] = self.channel_1_save[0]
            self.channel_4_save[0] = self.channel_1_save[0]

            self.ch_2_scalevar_a.set(self.channel_2_save[0])
            self.ch_3_scalevar_a.set(self.channel_3_save[0])
            self.ch_4_scalevar_a.set(self.channel_4_save[0])
    ...
    def current_multi_group_B(self, event=None):
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.channel_1_save[0] = self.channel_2_save[0]
            self.channel_3_save[0] = self.channel_2_save[0]
            self.channel_4_save[0] = self.channel_2_save[0]

            self.ch_1_scalevar_a.set(self.channel_1_save[0])
            self.ch_3_scalevar_a.set(self.channel_3_save[0])
            self.ch_4_scalevar_a.set(self.channel_4_save[0])
    ...
    def current_multi_group_C(self, event=None):
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.channel_1_save[0] = self.channel_3_save[0]
            self.channel_2_save[0] = self.channel_3_save[0]
            self.channel_4_save[0] = self.channel_3_save[0]
    ...
    def current_multi_group_D(self, event=None):
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.channel_1_save[0] = self.channel_4_save[0]
            self.channel_2_save[0] = self.channel_4_save[0]
            self.channel_3_save[0] = self.channel_4_save[0]
    ...
    def const_intensity_group_A(self, event=None):
        ...
        if self.ch_sel_str == '1 - 4':
            ...
            self.channel_1_save, self.channel_2_save, self.channel_3_save, self.channel_4_save = self.RGBW_intensity_limit(1, self.channel_1_save, self.channel_2_save, self.channel_3_save, self.channel_4_save)
            self.percentage_intensity_function(self.channel_1_save, self.channel_2_save, self.channel_3_save, self.channel_4_save)
            self.param_to_machine(1, self.channel_1_save)
    ...
    def const_intensity_group_B(self, event=None):
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.channel_1_save, self.channel_2_save, self.channel_3_save, self.channel_4_save = self.RGBW_intensity_limit(2, self.channel_1_save, self.channel_2_save, self.channel_3_save, self.channel_4_save)
            self.percentage_intensity_function(self.channel_1_save, self.channel_2_save, self.channel_3_save, self.channel_4_save)
            self.param_to_machine(2, self.channel_2_save)
    ...
    def const_intensity_group_C(self, event=None):
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.channel_1_save, self.channel_2_save, self.channel_3_save, self.channel_4_save = self.RGBW_intensity_limit(3, self.channel_1_save, self.channel_2_save, self.channel_3_save, self.channel_4_save)
            self.percentage_intensity_function(self.channel_1_save, self.channel_2_save, self.channel_3_save, self.channel_4_save)
            self.param_to_machine(3, self.channel_3_save)
    ...
    def const_intensity_group_D(self, event=None):
        ...
        if self.ch_sel_str== '1 - 4':
            ...
            self.channel_1_save, self.channel_2_save, self.channel_3_save, self.channel_4_save = self.RGBW_intensity_limit(4, self.channel_1_save, self.channel_2_save, self.channel_3_save, self.channel_4_save)
            self.percentage_intensity_function(self.channel_1_save, self.channel_2_save, self.channel_3_save, self.channel_4_save)
            self.param_to_machine(4, self.channel_4_save)
    ...
    ###############################################################################################
    #9. SPECIAL FUNCTIONS (LC18 RGBW)
    def RGBW_intensity_limit(self, arr_index, ch1_save, ch2_save, ch3_save, ch4_save): #used for LC18 RGBW model. Firmware version is usually '4.0.X.X'

        intensity_diff = 0
        net_intensity_1 = net_intensity_2 = net_intensity_3 = net_intensity_4 = 0

        net_intensity_1 = ch1_save[1]
        net_intensity_2 = ch2_save[1]
        net_intensity_3 = ch3_save[1]
        net_intensity_4 = ch4_save[1]

        intensity_diff = 255 - net_intensity_1 - net_intensity_2 - net_intensity_3 - net_intensity_4

        if intensity_diff < 0:
            if arr_index == 1:
                ch1_save[1] = int(net_intensity_1 + intensity_diff)
                self.ch_1_scalevar_b.set(ch1_save[1])

            elif arr_index == 2:
                ch2_save[1] = int(net_intensity_2 + intensity_diff)
                self.ch_2_scalevar_b.set(ch2_save[1])

            elif arr_index == 3:
                ch3_save[1] = int(net_intensity_3 + intensity_diff)
                self.ch_3_scalevar_b.set(ch3_save[1])

            elif arr_index == 4:
                ch4_save[1] = int(net_intensity_4 + intensity_diff)
                self.ch_4_scalevar_b.set(ch4_save[1])

        else:
            pass
        return ch1_save, ch2_save, ch3_save, ch4_save

    def percentage_intensity_function(self, ch1_save, ch2_save, ch3_save, ch4_save):
        total = 0
        total_percent = 0
        net_intensity_1 = net_intensity_2 = net_intensity_3 = net_intensity_4 = 0
        ...
        total = net_intensity_1 + net_intensity_2 + net_intensity_3 + net_intensity_4
        total_percent = np.multiply(np.divide(total, 255), 100)

        total_intensity = round(total_percent, 2)

        self.canvas_progress = tk.Canvas(self.total_intensity_frame, highlightthickness=0) #highlightcolor = 'white'
        self.canvas_progress['bg'] = 'DarkSlateGray2'
        self.canvas_progress['width'] = self.total_intensity_frame.winfo_reqwidth() - 5
        self.canvas_progress['height'] = self.total_intensity_frame.winfo_reqwidth() - 5
        #self.canvas_progress.place(relx=0.125, rely = 0.16)
        self.canvas_progress.place(relx=0, rely = 0.16)
        self.progressbar = CircularProgressbar(self.canvas_progress, x0 = 0, y0 = 0
            , x1 = self.total_intensity_frame.winfo_reqwidth(), y1 = self.total_intensity_frame.winfo_reqwidth(), intensity_percent = total_intensity, width = 12)