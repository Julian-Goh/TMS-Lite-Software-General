...
class LC18_OD_GUI(tk.Frame):
    ...
    def ch_ctrl_panel(self, prnt, panel_name):
        ...
        #####################################################################################################################
        ch_label_d = WrappingLabel(ch_sett_gui, text = 'Strobe Width (0-1000)', font = 'Helvetica 12', justify = 'right', anchor = 'ne')
        ...
        ch_scl_d = tk.Scale(ch_sett_gui, from_=0, to=1000, variable=scl_var_d, orient='horizontal', showvalue=0)

        entry_d_gui = tk.Frame(ch_sett_gui)
        entry_d_gui.grid_columnconfigure(index = 0, weight = 1, min = 70)

        ch_sbox_d = CustomSpinbox(master = entry_d_gui, width = 4, textvariable = sbox_var_d, from_=0, to= 1000, increment = 1
                             , highlightbackground="black", highlightthickness=1, font = 'Helvetica 12', input_rate = 10)
        
        Validate_Int(ch_sbox_d, sbox_var_d, only_positive = True, lo_limit = 0, hi_limit = 1000)
