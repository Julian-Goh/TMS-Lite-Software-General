...
from Tk_Custom_Widget.circular_progress_bar import CircularProgressbar
...
class LC18_RGBW_GUI(tk.Frame):
    ...
    def hmap_param_init(self):
        ...
        for i in range(1, 5):
            ...
            self.hmap_param['{}'.format(i)] = hashmap_init(ch_index)
    ...
    def progressbar_gen(self):
        self.total_intensity_fr = tk.Frame(self.left_anchor_fr, highlightcolor = 'white', bg = 'DarkSlateGray2', highlightbackground = 'white',  highlightthickness = 1)
        self.total_intensity_fr['width'] = 125

        self.total_intensity_fr.grid(column = 0, row = 2, columnspan = 1, rowspan = 1, padx = (5,5), pady = (5,5), sticky = 'nwse')

        self.total_intensity_fr.grid_columnconfigure(index = 0, weight = 1)

        tk_lb = tk.Label(self.total_intensity_fr, text = 'Total Intensity', font = 'Helvetica 12 bold', bg = 'DarkSlateGray2', anchor = 'nw')
        tk_lb.grid(column = 0, row = 0, columnspan = 1, rowspan = 1, padx = (5,5), pady = (5,5), sticky = 'we')

        self.progress_bar = CircularProgressbar(self.total_intensity_fr, highlightthickness=0)
        self.progress_bar['bg'] = 'DarkSlateGray2'
        self.progress_bar['width'] = 125
        self.progress_bar['height'] = 125
        self.progress_bar.grid(column = 0, row = 1, columnspan = 1, rowspan = 1, padx = (5,5), pady = (5,5), sticky = 'nwse')
    ...
    def ch_setting_gen(self):
        ...
        # self.EEPROM_btn.place(x= 270, y = 60)
    ...
    def strobe_ch_sel(self):
        ...
        for tk_var in self.ch_sel_var_hmap.values():
            ...
            ##### We only have 4 channels, so we disable the other tk_cbtn
            if i > 4:
                tk_cbtn['state'] = "disabled"
            #####
            if (i - 1)%4 == 0 and i > 1:
                row_j = 0
                col_j += 1
    ...
    def out_multiplier(self, event, id_num, save = False):
        if self.__read_mode == False:
            ...
            if type(save) == bool:
                ...
                self.rgbw_sync_current(scl_var)
    ...
    def out_intensity(self, event, id_num, save = False):
        if self.__read_mode == False:
            ...
            self.rgbw_intensity_limit(scl_var)
    ...
    ###############################################################################################
    ### RGBW SPECIAL FEATURES
    def rgbw_intensity_limit(self, scl_var): #used for LC18 RGBW model. Firmware version is usually '4.0.X.X'
        curr_net = 0 ## get the current net intensity
        for hmap_data in self.hmap_param.values():
            curr_net = curr_net + int(hmap_data['intensity']['widget'].get())

        intensity_diff = 255 - curr_net 

        if intensity_diff < 0:
            new_value = int(scl_var.get()) + intensity_diff

            scl_var.set(new_value)

            net_intensity = curr_net + intensity_diff
            percent = round((net_intensity/255 * 100), 2)
            self.progress_bar.update_bar(percent)

        else:
            net_intensity = curr_net
            percent = round((net_intensity/255 * 100), 2)
            self.progress_bar.update_bar(percent)

    def rgbw_sync_current(self, scl_var):
        value = scl_var.get()
        for hmap_data in self.hmap_param.values():
            if  hmap_data['multiplier']['widget'] != scl_var:
                hmap_data['multiplier']['widget'].set(value)
                addr_id = 'a'
                ch_index = hmap_data['ch_index']

                if value != hmap_data['multiplier']['value']:
                    hmap_data['multiplier']['value'] = value
                    self.ctrl.select_address(self.hmap_addr[addr_id])
                    self.ctrl.set_multiplier(ch_index, value)
    ...
    def reset_event(self): #Functions used in RESET ALL which switches off all the lights as well
        ...
        self.progress_bar.update_bar(0)
    ...
    def refresh_event(self, event=None):
        ...
        for hmap_id, hmap_data in self.hmap_param.items():
            ...
            intensity_val = hmap_data['intensity']['value']
            if is_int(intensity_val) == True:
                curr_net = curr_net + intensity_val
