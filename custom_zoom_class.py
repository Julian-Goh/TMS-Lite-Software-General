...
class CanvasImage():
    ...
    def __init__(self, placeholder, loaded_img = None, local_img_split = False, ch_index = 0): 
        ...
        # self.imframe.place(x=0, y=0, relheight=1, relwidth=1, anchor = 'nw')
    ...
    def ROI_box_enable(self, ivs_mode = None):
    ...
    def ROI_box_img_update(self):
        ...
        return self.roi_box[0], self.roi_box[1], self.loaded_img, self.roi_bbox_img, self.imscale, self.box_image[0], self.box_image[1], self.draw_save_img, self.crop_offset
    ...
    def ROI_line_enable(self, ivs_mode = None):
        _enable_status = False
        if ((isinstance(self.loaded_img, np.ndarray)) == True) and self.canvas_hide_img == False:
    ...
    def ROI_line_mouse_up(self, event):
        if self.roi_item:
            ...
            del ref_img
        # self.canvas.unbind_all('<B3-Motion>')
